...
import { parseCanonicalJsonText } from "../compiler/scene-contract.mjs";
import {
...
test("candidate lock keeps repositories independent", async () => {
  ...
  assert.equal(lock.candidates.candidate01.architectureBaseline.commit, "df564befcd65cb51a345fa9d315e40cadef6e563");
  assert.equal(lock.candidates.candidate01.architectureBaseline.specificationSha256, "29d76ca0feaefd4bf9cac9ebd25113c601e358c939778c4a0f43f3f94b58e0dd");
  assert.equal(lock.candidates.candidate01.release, null);
});

test("candidate lock and readiness parsing reject duplicate keys and noncanonical encodings", async () => {
  const lockText = await readFile(resolve(root, "experiment/warm-modern-meeting-room/candidate-lock.json"), "utf8");
  assert.equal(parseCandidateLockText(lockText).exterior.commit, "380098d4b7cbc1d57498b059466f095ae3568929");
...
test("compiler source attestation covers every trusted build input and rejects omissions", () => {
  assert.deepEqual(compilerSourceAttestationPaths, [
  ...
  const complete = Object.fromEntries(compilerSourceAttestationPaths.map((path) => [path, "a".repeat(64)]));
  assert.deepEqual(validateCompilerSourceAttestation(complete), complete);
  const omitted = { ...complete };
  delete omitted["schemas/scene-spec.schema.json"];
  assert.throws(() => validateCompilerSourceAttestation(omitted), /compiler_source_attestation_invalid/);
});
...
test("readiness records the approved internal GPU generation scope", async () => {
  ...
  assert.equal(readiness.stage3.roomMaterialZonesCompiled, true);
  assert.equal(readiness.stage3.roomMaterialRecipeCount, 3);
  assert.equal(readiness.stage3.roomTextureImagesCompiled, false);
  assert.equal(readiness.stage3.syntheticGlbExporterImplemented, true);
  ...
  assert.equal(readiness.stage3.approvedCandidateArchitectureBaselineCommit, "df564befcd65cb51a345fa9d315e40cadef6e563");
  assert.equal(readiness.stage3.approvedCandidateArchitectureBaselineGitBlobInputCount, 4);
  assert.equal(readiness.stage3.approvedCandidateArchitectureSemanticSha256, "ae24faad5306191667195c0157db9cd5c6d800875492cdf242fe32d1ff962b33");
  assert.equal(readiness.stage3.approvedCandidateComponentsCompilerImplemented, true);
  assert.equal(readiness.stage3.approvedCandidateComponentsCompileApi, "compileApprovedCandidateComponents");
  assert.equal(readiness.stage3.approvedCandidateComponentsReproducibilityApi, "verifyApprovedCandidateComponentsReproducibility");
  assert.equal(readiness.stage3.approvedCandidateComponentsSpecified, true);
  assert.equal(readiness.stage3.approvedCandidateComponentsCompiled, true);
  assert.deepEqual([
    readiness.stage3.approvedCandidateComponentCount,
    readiness.stage3.approvedCandidateComponentPartObjectCount,
    readiness.stage3.approvedCandidateComponentMeshCount,
    readiness.stage3.approvedCandidateComponentMaterialCount
  ], [11, 38, 57, 5]);
  assert.equal(readiness.stage3.approvedCandidateComponentGlbByteIdenticalVerified, true);
  assert.equal(readiness.stage3.approvedCandidateComponentGlbSha256, "a6e67219590ae4bbc1e887f97f9a7c071c924943223a90ef3560bdd7b06e5c69");
  assert.equal(readiness.stage3.approvedCandidateComponentGlbByteLength, 557976);
  assert.equal(readiness.stage3.approvedCandidateComponentReopenInspectionSha256, "5a1014f9fad8f12929d43d7fae0fd8155274754dc514db6590151bbbcda5e810");
  assert.deepEqual([
    readiness.stage3.approvedCandidateComponentBinaryByteLength,
    readiness.stage3.approvedCandidateComponentDecodedVertexCount,
    readiness.stage3.approvedCandidateComponentDecodedIndexCount,
    readiness.stage3.approvedCandidateComponentDecodedTriangleCount,
    readiness.stage3.approvedCandidateComponentDistinctPositionCount,
    readiness.stage3.approvedCandidateComponentDecodedNormalCount
  ], [485448, 15120, 22284, 7428, 3824, 15120]);
  assert.deepEqual(readiness.stage3.approvedCandidateComponentKhronosValidation, {
  ...
  assert.equal(readiness.stage3.approvedCandidateComponentBaselineCommit, "8fec157a37bf619797f1ff200ccc32f611f94c18");
  assert.equal(readiness.stage3.approvedCandidateMediaSurfaceBaselineCommit, "26d3af6e2720576113431c22b9443533b919f390");
  assert.equal(readiness.stage3.approvedCandidateMediaSurfaceBaselineGitBlobInputCount, 6);
  assert.equal(readiness.stage3.approvedCandidateMediaSurfaceSourceLoaderApi, "loadApprovedCandidateMediaSurfaceSource");
  assert.equal(readiness.stage3.approvedCandidateMediaSurfacesCompileApi, "compileApprovedCandidateMediaSurfaces");
  assert.equal(readiness.stage3.approvedCandidateMediaSurfacesReproducibilityApi, "verifyApprovedCandidateMediaSurfacesReproducibility");
  assert.equal(readiness.stage3.approvedCandidateMediaSurfaceCount, 2);
  assert.equal(readiness.stage3.approvedCandidateMediaSurfaceProjectionSha256, "352b31af533049d7fe84f1ecb55643db85e7258ceff1e2d87be8f8785e38a4fb");
  assert.equal(readiness.stage3.approvedCandidateMediaSurfaceProjectionByteLength, 1022);
  assert.equal(readiness.stage3.approvedCandidateMediaSurfaceProjectionRepresentation, "platform-runtime-plane");
  ...
  assert.equal(readiness.stage3.exteriorConstructionContractStatus, "candidate-source-validated-and-compiled");
  assert.equal(readiness.stage3.exteriorConstructionFixtureOnly, false);
  ...
  assert.equal(readiness.stage3.approvedCandidateExteriorSpecified, true);
  assert.equal(readiness.stage3.approvedCandidateExteriorCompiled, true);
  assert.deepEqual([
    readiness.stage3.approvedCandidateExteriorArchitectureMeshCount,
    readiness.stage3.approvedCandidateExteriorComponentMeshCount,
    readiness.stage3.approvedCandidateExteriorObjectCount,
    readiness.stage3.approvedCandidateExteriorMeshCount,
    readiness.stage3.approvedCandidateExteriorOutputMaterialCount
  ], [19, 38, 4, 61, 8]);
  assert.equal(readiness.stage3.approvedCandidateExteriorGlbByteIdenticalVerified, true);
  assert.equal(readiness.stage3.approvedCandidateExteriorGlbSha256, "eb74ca5e90b7dd09ad137c2127a53988491a557eb1d634093dd2b5eee6456b92");
  assert.equal(readiness.stage3.approvedCandidateExteriorGlbByteLength, 614784);
  assert.equal(readiness.stage3.approvedCandidateExteriorBlendByteIdentical, false);
  assert.equal(readiness.stage3.approvedCandidateExteriorReopenInspectionSha256, "d54209a0bb1c473910e701625f253d62fae5f70b3794dc04a8afeb3bd00f9f89");
  assert.deepEqual([
    readiness.stage3.approvedCandidateExteriorBinaryByteLength,
    readiness.stage3.approvedCandidateExteriorDecodedVertexCount,
    readiness.stage3.approvedCandidateExteriorDecodedIndexCount,
    readiness.stage3.approvedCandidateExteriorDecodedTriangleCount,
    readiness.stage3.approvedCandidateExteriorDistinctPositionCount,
    readiness.stage3.approvedCandidateExteriorDecodedNormalCount
  ], [535728, 16656, 24540, 8180, 4208, 16656]);
  assert.deepEqual(readiness.stage3.approvedCandidateExteriorKhronosValidation, {
    package: "gltf-validator",
    version: "2.0.0-dev.3.10",
    errors: 0,
    warnings: 0,
