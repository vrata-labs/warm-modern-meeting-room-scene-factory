...
#
# MODIFICATION NOTICE: This file differs from FlexiCubes commit
# 815e075a2a400d06c48d94c347674344ed6ae5c5. The shipped variant is
# inference-only, replaces external validation with local tensor checks, and
# removes differentiable triangulation and regularization result paths.
import torch
from .tables import *
...
def _require_tensor(name, value, shape, *, integer=False, device=None):
    ...
    if device is not None and value.device != device:
        raise ValueError(f"{name} must be on the same device as the vertex grid")
...
class FlexiCubes:
    ...
    def __call__(self, voxelgrid_vertices, scalar_field, cube_idx, resolution, qef_reg_scale=1e-3,
        ...
        _require_tensor("voxelgrid_vertices", voxelgrid_vertices, (None, 3))
        device = voxelgrid_vertices.device
        num_vertices = voxelgrid_vertices.shape[0]
        _require_tensor("scalar_field", scalar_field, (num_vertices,), device=device)
        _require_tensor("cube_idx", cube_idx, (None, 8), integer=True, device=device)
        if cube_idx.numel() == 0 or bool(torch.any(cube_idx < 0)) or bool(torch.any(cube_idx >= num_vertices)):
            raise ValueError("cube_idx contains an out-of-bounds vertex index")
        num_cubes = cube_idx.shape[0]
        if beta is not None:
            _require_tensor("beta", beta, (num_cubes, 12), device=device)
        if alpha is not None:
            _require_tensor("alpha", alpha, (num_cubes, 8), device=device)
        if gamma_f is not None:
            _require_tensor("gamma_f", gamma_f, (num_cubes,), device=device)
        if voxelgrid_colors is not None:
            _require_tensor("voxelgrid_colors", voxelgrid_colors, (num_vertices, None), device=device)
        ...
        vd, vd_gamma, vd_idx_map, vd_color = self._compute_vd(
        ...
        vertices, faces, vertices_color = self._triangulate(
    ...
    def _compute_vd(self, voxelgrid_vertices, surf_cubes_fx8, surf_edges, scalar_field,
        ...
        edge_group, edge_group_to_vd, edge_group_to_cube, vd_gamma = [], [], [], []
        ...
        return vd, vd_gamma, vd_idx_map, vd_color
    ...
    def _triangulate(self, scalar_field, surf_edges, vd, vd_gamma, edge_counts, idx_map, vd_idx_map, surf_edges_mask, vd_color):
        ...
        return vd, faces.reshape(-1, 3), vd_color
