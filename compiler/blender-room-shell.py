...
def apply_plan(plan, specification_sha256):
    try:
        import bpy
    except ImportError:
    ...
    for expected in plan["objects"]:
        actual_objects.append(verify_object(bpy.data.objects.get(expected["name"]), expected))
    return bpy, version, build_hash, binary_sha256, actual_objects
...
def inspect_current_blend(report_path, specification_sha256):
    try:
        ...
        fail("blender_python_required")
    version, build_hash, binary_sha256 = blender_identity(bpy)
    plan = build_shell_plan(load_exact_fixture())
    expected_names = {record["name"] for record in plan["objects"]}
    if set(bpy.data.objects.keys()) != expected_names \
            or len(bpy.data.meshes) != plan["meshCount"] \
    ...
    objects = [verify_object(bpy.data.objects.get(record["name"]), record) for record in plan["objects"]]
    ...
    report = {
        "schemaVersion": 1,
        "status": "stage3-synthetic-room-shell-inspection-valid",
        "fixtureOnly": True,
...
def main(argv=None):
    ...
    if args.plan_only:
        ...
        report = {
            **base_report,
            "status": "stage3-synthetic-room-shell-plan-valid",
            "execution": "plan-only",
            "blender": None,
            "outputBlend": None,
        }
    ...
    bpy, version, build_hash, binary_sha256, actual_objects = apply_plan(plan, args.expected_specification_sha256)
    bpy.ops.wm.save_as_mainfile(filepath=str(output_path), check_existing=False, compress=False, relative_remap=False)
    output_bytes = output_path.read_bytes()
    report = {
        **base_report,
        "status": "stage3-synthetic-room-shell-compiled",
        "execution": "blender",
        "blender": {
            "version": version,
    ...
    report["shell"] = {**plan, "objects": actual_objects}
    write_report(report_path, report)
    print(json.dumps(report, ensure_ascii=True, sort_keys=True, separators=(",", ":")))
