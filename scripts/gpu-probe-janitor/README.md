...
snapshots, images, and addresses in the exact function folder. The boundary is
validated for the complete inventory before any mutation. It submits expired VM
deletions first. Attached disks and used addresses are deferred to the next
timer pass; snapshots are deleted only at their own expiry. Images, detached
disks, and unused reserved external addresses converge independently.

Every delete waits for the returned Yandex operation to complete. Provider 404
responses are success. Immediate or eventual operation errors, conflicts,
authorization errors, exhausted transient retries, or insufficient invocation
time fail the run so the trigger or next timer pass retries from fresh inventory.
...
5. verify zero experiment instances, disks, snapshots, images, and reserved
   external addresses after convergence;
